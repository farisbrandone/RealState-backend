...
import { FinancingSimulator } from "@/widgets/FinancingSimulator/FinancingSimulator";
import { PropertyVirtualTour } from "@/widgets/PropertyVirtualTour/PropertyVirtualTour";
...
export default function PropertyPageClient() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      {/* Titre, prix et actions — le prix et son badge de transaction sont
          protégés (whitespace-nowrap + shrink-0) : sans ça, sur mobile, le
          badge "À VENDRE" se compressait jusqu'à casser son propre texte sur
          deux lignes (voir capture utilisateur). Si le groupe prix+badge+
          partage ne tient toujours pas sur une ligne, flex-wrap fait
          descendre "Partager" proprement plutôt que de casser le badge. */}
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2 mb-1">
            <h1 className="text-2xl sm:text-3xl font-heading text-primary-900">
              {property.title}
            </h1>
            <span className="shrink-0 whitespace-nowrap text-xs sm:text-sm bg-primary-100 text-primary-600 px-2 py-1 rounded-full">
              {typeLabel}
            </span>
          </div>
          <p className="text-primary-500 flex items-center gap-1 mt-1 text-sm sm:text-base">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 md:justify-end md:shrink-0">
          <div className="flex flex-wrap items-baseline gap-2">
            <span className="whitespace-nowrap text-2xl sm:text-3xl font-heading text-accent">
              {formatPrice(listing.price.amount, listing.price.currency)}
            </span>
            <span className="shrink-0 whitespace-nowrap text-xs sm:text-sm font-semibold uppercase tracking-wide bg-primary-100 text-primary-600 px-3 py-1 rounded-full">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-8">
        ...
        <div className="space-y-6">
          ...
          {/* Simulateur de financement — uniquement pour les biens à vendre */}
          {listing.type === "sale" && (
            <FinancingSimulator
