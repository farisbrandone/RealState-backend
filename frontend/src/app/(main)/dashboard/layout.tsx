...
const navItems = [
  ...
  { label: 'Mes propriétés', href: '/dashboard/properties', icon: BuildingOfficeIcon },
  { label: 'Profil', href: '/dashboard/profile', icon: UserCircleIcon },
  { label: 'Sécurité', href: '/dashboard/profile/security', icon: ShieldCheckIcon },
  { label: 'Paiements', href: '/dashboard/payments', icon: CreditCardIcon },
  { label: 'Abonnements', href: '/dashboard/payments/subscriptions', icon: CreditCardIcon },
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen">
      ...
      {/* Mobile nav (simple select or toggle) – omitted for brevity */}
      <main className="flex-1 bg-background p-6 lg:p-10 overflow-y-auto">{children}</main>
    </div>
  );
}
